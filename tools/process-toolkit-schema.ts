...
/** A `string` type representing the path to a `JSON` File. */
type JsonFilePath = `${string}.json${'' | 'c'}`;
/** A `string` type representing the path to a `YAML` File. */
 ...
 */
const TOOLKIT_PATH = `${ROOT_PATH}/toolkit`;
 ...
 * 
 * Also note that this function does **not** attempt
 * to *validate* the Toolkit Schema. To validate
 * the Toolkit Schema, see {@link verifySchema `verifySchema()`}.
 * 
 ...
 * 
 * @see         {@link verifySchema `verifySchema()`}
 */
 ...
 */
const readYamlFile = ( filePath: YamlFilePath ) => YAML.parseDocument(readFile(filePath));
 ...
 */
const stringifyJson = ( data: Record<string, any> ): string => `${JSON.stringify(data, null, 2)}\n`;
/**
